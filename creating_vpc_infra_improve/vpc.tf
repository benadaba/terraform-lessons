...
resource "aws_subnet" "public_subnet-1" {
  count = length(var.public_cidrs_list) > 0 ? length(var.public_cidrs_list) : 0
  vpc_id     = aws_vpc.main_tf_vpc.id
  # cidr_block = var.public_cidrs_list[0]  
  cidr_block = element(concat(var.public_cidrs_list,["10.0.43.0/24"]), count.index)
...
resource "aws_route_table_association" "public-sub-1-assoc" {
  count =  length(var.public_cidrs_list) > 0 ? length(var.public_cidrs_list) : 0
  subnet_id      = element(aws_subnet.public_subnet-1[*].id, count.index)
